...
echo       PUBLIC IP
echo       ---------
echo       1) Geolocate                2) Trace DNS
echo        3) Port Scan              4) DDOS
echo         5) Trace Mac Address    6) Port Scan
echo         7) ARP-SRP (DOX)      8) RPC Dump
echo.
echo       DISCORD TOOL(STILL IN DEVLOPEMENT)
echo       ------------
echo       9) Nuke                10) VC join
echo        11) Server Scan      12) Mass Report
echo         13) Id-To-IP       14) Mass Dm
echo          15) Bot join     16)Mass Ping
set /p input=
...
if /I "%input%" EQU "9" goto nuke
if /I "%input%" EQU "10" goto vc join
if /I "%input%" EQU "11" goto server scan
if /I "%input%" EQU "12" goto mass report
if /I "%input%" EQU "13" goto id-to-ip
if /I "%input%" EQU "14" goto mass dm
if /I "%input%" EQU "15" goto bot join
if /I "%input%" EQU "16" goto mass ping
...
set errorlevel=0
set /p ip=Enter IP Address: 
start cmd /c "mode 87, 10 && title D0xing %ip%... && echo. && arpspoof.exe %ip%"
goto menu

:ddos
cls
echo.
echo 1) https://stresser.zone/
echo 2) https://hardstresser.com/
echo 3) https://stresserst.su/
echo 4) https://www.stressthem.se/
echo 5) https://stresse.net/
echo 6) Back
echo.
set /p ddosinput=
